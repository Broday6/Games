...
(function (G) {
  'use strict';
  const A = { ctx: null, master: null, muted: false, amb: null };
  G.Audio = A;
  A.init = function () {
    if (A.ctx) return;
    try { A.ctx = new (window.AudioContext || window.webkitAudioContext)(); } catch (e) { return; }
    A.master = A.ctx.createGain(); A.master.gain.value = 0.5; A.master.connect(A.ctx.destination);
  ...
  A.toggleMute = () => { A.muted = !A.muted; if (A.master) A.master.gain.value = A.muted ? 0 : 0.5; return A.muted; };
  ...
  function noise(dur, vol, freq, delay) {
    ...
    const f = c.createBiquadFilter(); f.type = 'bandpass'; f.frequency.value = freq || 1000; f.Q.value = 0.8; const g = c.createGain(); g.gain.setValueAtTime(vol || 0.2, t0); g.gain.exponentialRampToValueAtTime(0.001, t0 + dur);
    src.connect(f); f.connect(g); g.connect(A.master); src.start(t0);
  }
  const SFX = {
  ...
  A.play = function (name, x, y, lx, ly) {
    ...
    if (x !== undefined && lx !== undefined) { const d = G.dist(x, y, lx, ly); if (d > 18) return; }
