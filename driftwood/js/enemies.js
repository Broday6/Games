...
(function (G) {
  ...
  function moveToward(S, e, tx, ty, dt, spdMul) {
    const a = G.angleTo(e.x, e.y, tx, ty);
    const spd = e.spd * (spdMul || 1) * (e.slow > 0 ? 0.7 : 1) * G.tileSpeed(S.world, e.x, e.y);
    e.face = a;
  ...
  function spawnNear(S, p, type, minD, maxD, lvl) {
    for (let t = 0; t < 12; t++) {
      ...
      const e = Sim.makeEnemy(S, type, x, y, lvl); S.enemies.push(e); return e;
    }
    return null;
  }
  ...
  Enemies.update = function (S, dt) {
    ...
    if (players.length && S.phase === 'run') {
      ...
      if (inNight) {
        ...
        if (S.time >= G.NIGHT_AT + 28) wave('b', Math.round((5 + night * 3) * (0.6 + 0.4 * nPl)));
        S.spawnT -= dt;
        if (S.spawnT <= 0 && alive < cap) { S.spawnT = Math.max(2.5, 10 - night * 0.7) / Math.sqrt(nPl); const p = players[Math.floor(Math.random() * players.length)]; spawnNear(S, p, pickType(S, night, S.world.biome[G.idx(p.x, p.y)]), 9, 15); }
