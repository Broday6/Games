// DRIFTWOOD — main glue: lobby flow, game loop, host/client sync
(function (G) {
  'use strict';
  const M = { S: null, V: null, me: null, mode: null, started: false, pending: null, acc: 0, lastB: 0, lastIn: 0, snapPrev: null, snapCur: null, snapT: 0, pred: null, world: null, pingMs: 0 };
  G.Main = M;
  const STEP = 1 / 30, BCAST = 1 / 15, INRATE = 1 / 20;
  const Sim = G.Sim, Net = G.Net, R = G.Render, UI = G.UI, In = G.Input, A = G.Audio;

  window.addEventListener('DOMContentLoaded', () => {
    UI.init(); R.init(document.getElementById('game'), document.getElementById('minimap')); In.init(document.getElementById('game'));
    In.onAction = onLocalAction;
  ...
  function makeHostSim(name, col, seed) {
    ...
    Sim.addPlayer(M.S, 'host', name, col);
    updateLobbyPlayers();
  }
  M.host = function (name, col, seed) {
    makeHostSim(name, col, seed);
    const code = Net.makeCode();
    Net.host(code, (ok) => { if (ok) { UI.showHostInfo(code); const p = document.getElementById('players'); } else { UI.showHostInfo('—'); } });
  ...
  // ---------- networking ----------
  function onJoin(id) {
    if (M.mode === 'client') { Net.send('host', { t: 'hello', name: M.pending ? M.pending.name : 'Castaway', col: M.pending ? M.pending.col : '#fff' }); UI.status('Connected — waiting for host…'); }
  }
  function onLeave(id) {
  ...
  function onMessage(from, msg) {
    if (M.mode === 'host') {
      if (!M.S) return;
      if (msg.t === 'hello') {
        if (!M.S.players[from]) Sim.addPlayer(M.S, from, msg.name, msg.col);
        updateLobbyPlayers();
      ...
      else if (msg.t === 'start') { if (!M.started) { M.started = true; A.init(); A.resume(); UI.enterGame(M.world.seed); } }
  ...
  function applySnapshot(snap) {
    ...
    const enemies = snap.enemies.map(a => ({ id: a[0], t: G.EN_LIST[a[1]], x: a[2], y: a[3], hp: a[4], maxHp: a[5], st: a[6], face: a[7], flash: a[8], r: a[9], stun: a[10], hidden: a[11], pet: a[12], burn: a[13], tm: a[14] }));
    const projs = snap.projs.map(a => ({ id: a[0], type: a[1], x: a[2], y: a[3], a: a[4] }));
    const drops = snap.drops.map(a => ({ id: a[0], item: a[1] < 0 ? 'coin' : G.ITEM_LIST[a[1]], x: a[2], y: a[3], n: a[4] }));
    const puddles = snap.puddles.map(a => ({ x: a[0], y: a[1], r: a[2], t: a[3] }));
    M.snapPrev = M.snapCur; M.snapCur = { time: snap.time, day: snap.day, phase: snap.phase, siegeT: snap.siegeT, boat: snap.boat, bosses: snap.bosses, players: snap.players, enemies, projs, drops, puddles, stats: snap.stats, diff: snap.diff, at: performance.now() / 1000 };
  ...
  function handleEvents(list) {
    ...
    for (const ev of list) {
      ...
      switch (ev.t) {
        ...
        case 'end': A.play(ev.win ? 'win' : 'lose'); setTimeout(() => UI.end(ev.win, M.V), 1200); break;
        case 'sail': UI.confirm('<b>The ship is ready.</b><p>Setting sail will summon everything the island has left.<br>Hold the dock for 90 seconds, then kill what rises.<br>Make sure everyone is here and stocked up.</p>', () => M.act({ a: 'sail' })); break;
  ...
  function onLocalAction(a) {
    ...
    if (a.a === 'click') {
      const it = me.inv[me.held];
      if (it && G.ITEMS[it.id].type === 'place') { const tx = Math.floor(a.x), ty = Math.floor(a.y); M.act({ a: 'build', item: it.id, tx, ty }); }
      return;
    }
  ...
  // ---------- view construction ----------
  function hostView() {
    const S = M.S; const V = { world: S.world, time: S.time, day: S.day, phase: S.phase, siegeT: Math.ceil(S.siegeT), boat: S.boat, bosses: S.bosses, players: S.players, enemies: S.enemies, projs: S.projs, drops: S.drops, puddles: S.puddles, stats: S.stats, diff: Sim.difficulty(S), me: 'host', now: performance.now() / 1000, elapsed: S.elapsed };
  ...
  function clientView(dt) {
    ...
    for (const id in c.players) {
      ...
      if (id === M.me && M.pred && !cur.dead && !cur.downed) { q.x = M.pred.x; q.y = M.pred.y; const w = In.aim; q.face = G.angleTo(q.x, q.y, w.x, w.y); }
      players[id] = q;
    }
    ...
    return { world: M.world, time: c.time + (now - c.at), day: c.day, phase: c.phase, siegeT: c.siegeT, boat: c.boat, bosses: c.bosses, players, enemies, projs: c.projs, drops: c.drops, puddles: c.puddles, stats: c.stats, diff: c.diff, me: M.me, now, elapsed: 0, chestDisc: me ? Sim.stats(me).chestDisc : 1, netlbl: 'connected · ' + M.pingMs + 'ms' };
  }
  M.view = () => M.V;
  ...
  function predict(dt) {
    ...
    const inp = In.packet(); let ax = inp.ax, ay = inp.ay; const l = Math.hypot(ax, ay); if (l > 1) { ax /= l; ay /= l; }
    if (!l) return;
    ...
    G.moveCircle(M.world, M.pred, ax * spd * dt, ay * spd * dt, 0.3, false);
  }

  // ---------- hint ----------
  function hintFor(V) {
    const me = V.players[V.me]; if (!me || me.dead) return me && me.dead ? 'You are dead. You will wash ashore again at dawn.' : '';
    if (me.downed) return 'You are down! A teammate can revive you (hold E).';
    ...
    const it = me.inv[me.held]; if (it && G.ITEMS[it.id].type === 'place') return 'LMB: place ' + G.ITEMS[it.id].name;
    if (it && G.ITEMS[it.id].type === 'bow') return 'Hold RMB to draw, release to shoot (' + Sim.count(me, 'arrow') + ' arrows)';
    if (me.hunger < 25) return 'You are starving — eat something (Q)';
    return '';
  }
  ...
  function loop() {
    ...
    if (M.mode === 'host') {
      const S = M.S; M.acc += dt;
      let steps = 0;
      while (M.acc >= STEP && steps < 5) { Sim.setInput(S, 'host', In.packet()); Sim.step(S, STEP); Sim.flushChanges(S); M.acc -= STEP; steps++; }
      if (steps === 5) M.acc = 0;
      flushEvents();
      if (Net.count() && now - M.lastB >= BCAST) { M.lastB = now; Net.broadcast(Sim.snapshot(S, false)); }
      M.V = hostView();
    } else {
      if (now - M.lastIn >= INRATE) { M.lastIn = now; Net.send('host', { t: 'in', in: In.packet() }); if (Math.random() < 0.05) Net.send('host', { t: 'ping', k: performance.now() }); }
      predict(dt);
      M.V = clientView(dt);
      if (!M.V) return;
    }
    ...
    // build ghost
    let ghost = null;
    if (me && !me.dead) { const it = me.inv[me.held]; if (it && G.ITEMS[it.id].type === 'place') { const tx = Math.floor(In.aim.x), ty = Math.floor(In.aim.y); const d = G.ITEMS[it.id]; const od = G.OBJS[d.obj]; const t = G.tileAt(V.world, tx, ty); const ok = G.inWorld(tx, ty) && G.dist(me.x, me.y, tx + .5, ty + .5) <= 5.5 && !V.world.objs.has(G.idx(tx, ty)) && (od.floor ? (t === G.T.WATER || t === G.T.DEEP) : (t > G.T.WATER && t !== G.T.LAVA)); ghost = { obj: d.obj, tx, ty, ok }; } }
    UI.update(V, hintFor(V));
    R.frame(V, dt, { aim: In.aim, ghost });
