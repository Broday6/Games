...
const HOSTED_URL = process.env.DRIFTWOOD_URL || 'https://broday6.github.io/Games/driftwood/'; // public copy where room codes work
html = html.replace('<script src="js/util.js"></script>', '<script>window.__VERSION = ' + JSON.stringify(version) + '; window.__HOSTED_URL = ' + JSON.stringify(HOSTED_URL) + '; window.__LICENSES = ' + JSON.stringify(licenses) + ';</script>\n<script src="js/util.js"></script>');
...
// the artifact runs inside claude.ai's sandbox, which blocks the room-server WebSocket: flag it so the lobby explains and links out
